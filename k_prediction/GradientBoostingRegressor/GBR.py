...
X_raw = df[raw_cols].reset_index(drop=True)
...
# === 2. 划分训练测试集 ===
interval = 5
test_idx = np.arange(0, len(X_all), interval)
X_test = X_all.iloc[test_idx].reset_index(drop=True)
y_test = y.iloc[test_idx].reset_index(drop=True)
X_train = X_all.drop(index=test_idx).reset_index(drop=True)
...
train_sizes, train_scores, test_scores = learning_curve(
    best_model, X_all, y, train_sizes=np.linspace(0.1, 1.0, 5),
...
plt.tight_layout()
plt.savefig("plots/learning_curve_gbr.png", dpi=300)
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("plots/pred_vs_actual_gbr.png", dpi=300)
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("plots/error_distribution_gbr.png", dpi=300)
plt.close()
